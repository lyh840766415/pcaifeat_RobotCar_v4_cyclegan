...
def forward_att(point_cloud, is_training, bn_decay=None,OUTPUT_DIM=256):
    ...
    CLUSTER_SIZE=64
    k=20
    point_cloud = tf.reshape(point_cloud, [batch_num_queries, num_points,13])
    ...
    with tf.variable_scope('SNfeature') as sc:
        ...
        idx_ = tf.range(batch_num_queries) * num_points
        idx_ = tf.reshape(idx_, [batch_num_queries, 1, 1])
    ...
    with tf.variable_scope("point_attention"):
	    print(net)
	    point_weights = pixel_attention(net)
	    
	    point_att_map = tf.reshape(point_weights, [-1, 4096])

	    point_weights = tf.reshape(point_weights, [-1, 1])
	    
	    print(point_weights)
	    
	    net= tf.reshape(net,[-1,1024])
	    net = tf.nn.l2_normalize(net,1)
	    output,_ = Att_NetVLAD_forward(point_weights, net, feature_size=1024, max_samples=4096, cluster_size=64,output_dim=256, gating=True, add_batch_norm=True,is_training=is_training)

	    output = tf.nn.l2_normalize(output,1)
	    print(output)
    return output,point_att_map


#Adopted from the original pointnet code
def forward(point_cloud, is_training, bn_decay=None,OUTPUT_DIM=256):
    """LPD-Net:FNSF,    INPUT is batch_num_queries X num_pointclouds_per_query X num_points_per_pointcloud X 13,
                        OUTPUT batch_num_queries X num_pointclouds_per_query X output_dim """
    batch_num_queries = point_cloud.get_shape()[0].value
    #num_pointclouds_per_query = point_cloud.get_shape()[1].value
    ...
    output = tf.nn.l2_normalize(output,1)
    output =  tf.reshape(output,[batch_num_queries,OUTPUT_DIM])
