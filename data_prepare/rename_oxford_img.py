...
OUTPUT_PATH = "/data/lyh/RobotCar/benchmark_datasets/oxford_day_night/"
	...
	for seq_id,seq in enumerate(all_seqs):
		...
		for pointcloud in all_pointclouds_1:
			...
			if seq_id in night_seq_id:
				output_filename = os.path.join(OUTPUT_PATH,seq,'pointcloud_20m_10overlap',"%s_night.png"%pointcloud[:-4])
			else:
				output_filename = os.path.join(OUTPUT_PATH,seq,'pointcloud_20m_10overlap',"%s_day.png"%pointcloud[:-4])
		...
		for pointcloud in all_pointclouds_2:
			...
			if seq_id in night_seq_id:
				output_filename = os.path.join(OUTPUT_PATH,seq,'pointcloud_20m',"%s_night.png"%pointcloud[:-4])
			else:
				output_filename = os.path.join(OUTPUT_PATH,seq,'pointcloud_20m',"%s_day.png"%pointcloud[:-4])
